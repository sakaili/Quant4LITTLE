# Quant4Little - 加密货币量化交易系统

一个基于技术指标和Transformer深度学习模型的加密货币量化交易系统，专注于Binance USDT永续合约市场的低流动性标的。

## 📋 目录

- [系统概览](#系统概览)
- [核心特性](#核心特性)
- [快速开始](#快速开始)
- [系统架构](#系统架构)
- [部署指南](#部署指南)
- [交易策略](#交易策略)
- [模型使用](#模型使用)
- [风险控制](#风险控制)
- [监控运维](#监控运维)
- [常见问题](#常见问题)

---

## 系统概览

### 项目定位

本系统是一个完整的量化交易框架，适用于Binance USDT永续合约市场。系统采用**小时级信号检测**架构，结合传统技术指标和深度学习模型，实现自动化的候选筛选、信号检测、订单执行和风险管理。

### 技术栈

- **语言**: Python 3.10+
- **交易接口**: ccxt
- **深度学习**: PyTorch, Transformer
- **数据处理**: pandas, numpy
- **部署**: Linux Cron (支持ECS/Ubuntu)

### 系统工作流

```
每天09:00 → 筛选候选池(300个币) → 候选列表CSV
每小时整点 → 检测信号(KDJ+EMA+ATR) → Transformer模型排序 → 并行下单(Top-10)
持仓管理 → 止盈止损监控 → 触发止盈/止损平仓
```

---

## 核心特性

### 1. 智能候选筛选
- 基于成交额倒数筛选低流动性标的
- 成交量过滤(>500万USDT)
- 自动生成每日候选池(300个标的)

### 2. 小时级信号检测
- 实时监控候选池
- 多指标信号确认:
  - KDJ反弹信号(J > 50)
  - EMA底部形态(EMA10 < EMA20 < EMA30)
  - ATR波动率过滤(≥2%)

### 3. Transformer模型排序
- 15维表格特征:
  - EMA指标: ema10, ema20, ema30及其比率
  - ATR波动率: atr14, atr14_over_close
  - 市场特征: funding_rate, log_quote_volume, log_market_cap
  - 收益特征: ret_mean_5d, ret_std_5d
  - 斜率特征: ema10_slope, ema20_slope

- 24×5时序特征(24小时K线):
  - close, volume, ema20, ema30, atr14

- 输出:
  - expected_value: 期望收益分数(-0.5到0.5)
  - pred_class: 预测类别(0=差, 1=中, 2=优)
  - 三类概率分布

- 自动回退机制: 模型失败时自动回退到KDJ排序

### 4. Maker订单策略
- 限价单挂单获取返佣(0.02%)
- 0.10%偏移量优化成交率
- 5次渐进式追单重试:
  - 0.10% → 0.08% → 0.05% → 0.03% → 0.01%
  - 最终市价兜底(0%)
- 60秒超时自动取消

### 5. 并行执行引擎
- ThreadPoolExecutor并发下单(5个worker)
- 智能资金管理(每笔2%名义价值: 1%净值×2倍杠杆)
- 最大持仓限制(10个品种)
- 实时成交统计

### 6. 风险管理
- 单品种资金限制: 2%名义价值(1%净值×2倍杠杆)
- 最大持仓数: 10个品种
- 杠杆倍数: 2x
- 止损止盈: 30%止盈 / 150%止损
- 持仓监控: 实时显示当前所有持仓

---

## 快速开始

### 环境准备

```bash
# 1. 克隆仓库
git clone https://github.com/xxx/Quant4Little.git
cd Quant4Little

# 2. 创建虚拟环境
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 3. 安装依赖
pip install -r requirements.txt

# 4. 配置环境变量
export BINANCE_API_KEY="你的API密钥"
export BINANCE_API_SECRET="你的API密钥"
export USE_TESTNET="False"  # True为测试网
export HTTPS_PROXY="http://127.0.0.1:7890"  # 可选代理
```

### 本地测试

#### 1. 回测验证策略

```bash
python scripts/backtester.py \
  --start 2025-01-01 \
  --end 2025-11-01 \
  --max-positions 30 \
  --bottom-n 300
```

输出包含:
- 权益曲线图
- 交易明细CSV
- Sharpe比率
- 最大回撤

#### 2. 生成候选池

```bash
python scripts/daily_candidate_scan.py --bottom-n 300
```

生成文件: `data/daily_scans/candidates_YYYYMMDD.csv`

#### 3. 检测小时级信号(使用模型)

```bash
python scripts/hourly_signal_detector.py \
  --use-model \
  --top-k 10
```

生成文件: `data/hourly_signals/signals_YYYYMMDD_HH.csv`

#### 4. 纸面交易测试

```bash
python scripts/live_maker_trader.py
# 按提示确认执行
```

---

## 系统架构

### 目录结构

```
Quant4Little/
├── scripts/                    # 核心脚本
│   ├── data_fetcher.py        # 数据获取(支持主网/测试网)
│   ├── daily_candidate_scan.py # 每日候选池筛选
│   ├── hourly_signal_detector.py # 小时级信号检测+模型排序
│   ├── live_maker_trader.py   # 实盘Maker订单执行
│   ├── backtester.py          # 事件驱动回测器
│   ├── indicator_utils.py     # 技术指标工具(KDJ等)
│   ├── lightweight_ranker.py  # 轻量级模型推理器
│   └── modeling/              # 深度学习模块
│       ├── model.py           # Transformer模型定义
│       ├── features.py        # 特征工程
│       └── train_ranker.py    # 模型训练
├── deploy/                     # 部署脚本
│   ├── clear_crontab.sh       # 清理定时任务
│   ├── setup_cron_hourly.sh   # 安装小时级定时任务
│   └── cron_hourly_detect.sh  # 小时级检测cron脚本
├── data/                       # 数据目录(被.gitignore)
│   ├── daily_klines/          # 日线数据
│   ├── hourly_klines/         # 小时线数据
│   ├── daily_scans/           # 候选池CSV
│   ├── hourly_signals/        # 小时信号CSV
│   └── backtest_trades.csv    # 交易记录(训练数据)
├── models/                     # 模型文件
│   ├── rank_model.pt          # PyTorch模型权重
│   ├── rank_model.onnx        # ONNX优化模型(可选)
│   └── rank_model_meta.json   # 模型元数据(归一化参数)
├── logs/                       # 日志目录
├── tests/                      # 测试脚本
└── README.md                   # 本文档
```

### 数据流

```
[Binance API]
    ↓
[data_fetcher.py] → 获取K线数据
    ↓
[daily_klines/] [hourly_klines/] → 存储历史数据
    ↓
[daily_candidate_scan.py] → 筛选候选池
    ↓
[daily_scans/candidates_YYYYMMDD.csv] → 候选列表
    ↓
[hourly_signal_detector.py] → 检测信号
    ↓                              ↓
[indicator_utils.py]          [modeling/features.py]
    ↓                              ↓
[技术指标过滤]               [构建模型特征]
    ↓                              ↓
[传统信号] ←─────────→ [lightweight_ranker.py]
    ↓                              ↓
                [Transformer模型排序]
                        ↓
        [hourly_signals/signals_YYYYMMDD_HH.csv]
                        ↓
            [live_maker_trader.py]
                        ↓
                [并行下单+持仓管理]
                        ↓
                [Binance订单簿]
```

---

## 部署指南

### ECS服务器部署(推荐)

#### 第一步: 拉取代码

```bash
cd ~/Quant4Little
git pull origin main
```

#### 第二步: 清理旧定时任务

```bash
cd ~/Quant4Little
bash deploy/clear_crontab.sh
```

脚本会:
- 自动备份当前crontab到`/tmp/crontab_backup_*.txt`
- 删除所有Quant4Little相关任务
- 显示清理结果

#### 第三步: 安装新定时任务

```bash
bash deploy/setup_cron_hourly.sh
# 显示将要安装的任务后，输入 y 确认
```

安装的定时任务:

```bash
# 每天09:00 (UTC 01:00) - 筛选候选币池
0 1 * * * cd ~/Quant4Little && python3 scripts/daily_candidate_scan.py --top-n 100 >> logs/cron_candidates.log 2>&1

# 每小时整点 - 检测信号并执行交易(使用Transformer模型)
0 * * * * cd ~/Quant4Little && bash deploy/cron_hourly_detect.sh >> logs/cron_hourly.log 2>&1
```

#### 第四步: 验证安装

```bash
# 查看已安装的定时任务
crontab -l

# 创建日志目录
mkdir -p ~/Quant4Little/logs

# 确保脚本可执行
chmod +x ~/Quant4Little/deploy/*.sh

# 手动测试候选池筛选
python3 scripts/daily_candidate_scan.py --top-n 100

# 手动测试信号检测(需要先有候选池)
python3 scripts/hourly_signal_detector.py --use-model --top-k 10
```

#### 第五步: 监控运行

```bash
# 实时监控小时级检测日志
tail -f logs/cron_hourly.log

# 实时监控候选池筛选日志
tail -f logs/cron_candidates.log

# 查看系统cron日志
sudo grep CRON /var/log/syslog | tail -20

# 查看生成的信号文件
ls -lh data/hourly_signals/
```

### 执行时间表(北京时间)

| 时间 | 任务 | 说明 |
|------|------|------|
| 09:00 | 筛选候选池 | 扫描全市场，选出300个候选币 |
| 10:00 | 检测信号+交易 | 使用Transformer模型排序，执行Top-10 |
| 11:00 | 检测信号+交易 | 同上 |
| ... | ... | 每小时重复 |
| 23:00 | 检测信号+交易 | 最后一次 |

---

## 交易策略

### 候选池筛选逻辑

每天09:00执行，筛选条件:

1. **流动性过滤**:
   - 按24小时成交额倒数排序，选取底部300个标的作为"空气币池"

2. **技术指标过滤**:
   - EMA排列: EMA10 < EMA20 < EMA30 (底部形态)
   - ATR波动率无极端暴涨(ATR < 3倍前3日均值)

3. **风险控制过滤**:
   - 排除主流币(BTC, ETH, BNB, SOL等)
   - 资金费率 > -1% (避免极端负费率)

**注**: 候选池不再使用KDJ过滤，预计产出100-150个标的，由小时级信号检测负责精准筛选。

### 信号检测逻辑

每小时整点执行，从候选池检测:

1. **技术信号确认**:
   - KDJ_J > 70 (强超买，做空信号)
   - EMA10 < EMA20 < EMA30 (仍在底部)

2. **模型智能排序**(可选):
   - 对所有技术信号进行模型评分
   - 计算expected_value (期望收益)
   - 按分数降序排列

3. **Top-K选择**:
   - 默认选择Top-10个信号
   - 过滤掉模型分数<-0.9的无效信号
   - 模型失败时自动回退到KDJ降序排列

### Maker订单执行策略

**入场逻辑**:

```
当前市场状态:
  Ask (卖1): 7.305 USDT
  Bid (买1): 7.300 USDT

Maker策略(做空开仓):
  → 在Ask之上挂限价卖单
  → 价格: 7.305 × (1 + 0.10%) = 7.312 USDT
  → 等待60秒成交
  → 未成交则取消订单，重试更激进的价格
```

**追单策略**(5次重试):
1. 第1次: offset=0.10% (保守)
2. 第2次: offset=0.08%
3. 第3次: offset=0.05%
4. 第4次: offset=0.03%
5. 第5次: offset=0.01% (激进)
6. 最终: offset=0% (市价兜底)

**资金管理**:
- 每笔使用账户净值的1% × 2倍杠杆 = 2%名义价值
- 计算公式: position_value = equity × 0.01 × 2.0
- 杠杆: 2x

### KDJ指标计算

使用标准KDJ算法:

```
1. RSV = (Close - Lowest_Low_9) / (Highest_High_9 - Lowest_Low_9) × 100

2. K(today) = (2/3) × K(yesterday) + (1/3) × RSV(today)
   初始K值 = 50

3. D(today) = (2/3) × D(yesterday) + (1/3) × K(today)
   初始D值 = 50

4. J = 3 × K - 2 × D
```

实现位置: [scripts/indicator_utils.py](scripts/indicator_utils.py)

---

## 模型使用

### 模型架构

**TransformerRanker** - 多模态Transformer排序模型

- **表格特征分支**: 15维特征 → FC(64) → Dropout(0.3)
- **序列特征分支**: (24, 5) → Transformer Encoder → 池化
- **融合层**: 拼接 → FC(64) → ReLU → Dropout(0.3) → FC(3)
- **输出**: 3分类 (Class 0/1/2)

### 训练模型

```bash
python scripts/modeling/train_ranker.py \
  --candidates-dir data/daily_scans \
  --backtest-csv data/backtest_trades.csv \
  --daily-dir data/daily_klines \
  --hourly-dir data/hourly_klines \
  --output-dir models \
  --epochs 50 \
  --batch-size 64
```

生成文件:
- `models/rank_model.pt` - PyTorch模型权重
- `models/rank_model_meta.json` - 归一化参数和元数据

### 使用模型推理

#### 方法1: 命令行使用

```bash
# 对指定日期的候选进行排序
python scripts/use_ranker_model.py --date 20250120 --top-k 20
```

#### 方法2: 在代码中使用

```python
from scripts.lightweight_ranker import LightweightRanker
from scripts.modeling.features import build_tabular_vector, build_hourly_sequence
import pandas as pd
from pathlib import Path

# 加载模型
ranker = LightweightRanker(model_dir=Path("models"))

# 准备特征
df_daily = pd.read_csv("data/daily_klines/BTCUSDT_1d.csv")
df_hourly = pd.read_csv("data/hourly_klines/BTCUSDT_1h.csv")

tabular = build_tabular_vector(
    df_daily,
    signal_date=date(2025, 1, 20),
    funding_rate=0.0001,
    quote_volume=1000000,
    market_cap=500000000
)

sequence = build_hourly_sequence(
    df_hourly,
    cutoff=pd.Timestamp("2025-01-20 23:59", tz="UTC"),
    seq_len=24
)

# 推理
expected_value, pred_class, probs = ranker.predict(tabular, sequence)

print(f"期望收益: {expected_value:.4f}")
print(f"预测类别: {pred_class}")
print(f"概率分布: Class0={probs[0]:.2%}, Class1={probs[1]:.2%}, Class2={probs[2]:.2%}")
```

### 模型性能

当前性能指标(需要根据实际训练结果更新):
- 验证集准确率: ~60-70% (取决于数据量)
- 测试集准确率: 建议至少收集500+样本
- Top-K收益改善: 相比传统排序提升5-10%

**重要提示**:
- 初始模型可能因样本不足导致泛化能力较弱
- 建议先累积足够的历史交易数据(>500笔)
- 定期用新数据重新训练模型
- 在纸面交易中验证至少1-2个月

### ONNX优化(可选)

如果服务器资源受限，可以转换为ONNX格式:

```bash
python scripts/convert_to_onnx.py
```

生成文件: `models/rank_model.onnx`

LightweightRanker会自动检测并优先使用ONNX模型。

---

## 风险控制

### 仓位管理

- **最大持仓数**: 10个品种
- **单品种资金**: 2%名义价值(1%净值×2倍杠杆)
- **杠杆倍数**: 2x
- **总风险敞口**: 最大20%名义价值(满仓时)

### 止损止盈

- **止盈**: 30% (触发后自动平仓)
- **止损**: 150% (触发后自动平仓)
- **平仓方式**: 仅通过止盈/止损触发，无时间限制

### 订单风险控制

1. **Maker订单超时**: 60秒未成交自动取消
2. **重试上限**: 最多5次追单重试
3. **最终兜底**: 市价单确保执行
4. **滑点控制**: 通过限价单避免大幅滑点

### 市场风险控制

1. **流动性过滤**: 成交量>500万USDT
2. **波动率过滤**: ATR≥2%
3. **排除主流币**: 避免与大盘高度相关
4. **候选池限制**: 每天只筛选300个标的

---

## 监控运维

### 日志管理

```bash
# 查看最近的小时级检测日志
tail -50 logs/cron_hourly.log

# 查看候选池筛选日志
tail -50 logs/cron_candidates.log

# 实时监控日志
tail -f logs/cron_hourly.log

# 查看系统cron执行日志
sudo grep CRON /var/log/syslog | grep Quant4Little
```

### 数据文件检查

```bash
# 查看候选池文件
ls -lh data/daily_scans/
cat data/daily_scans/candidates_20250124.csv

# 查看信号文件
ls -lh data/hourly_signals/
cat data/hourly_signals/signals_20250124_10.csv

# 查看历史交易记录
wc -l data/backtest_trades.csv
tail -20 data/backtest_trades.csv
```

### 持仓监控

```bash
# 查看当前持仓(如果实现了持仓管理脚本)
python scripts/manage_positions.py

# 查看账户余额
python scripts/check_balance.py
```

### 模型监控

```bash
# 查看模型文件修改时间(检查是否重训)
ls -lh models/

# 查看模型元数据
cat models/rank_model_meta.json | python -m json.tool

# 检查模型推理是否正常
python scripts/lightweight_ranker.py
```

### 性能统计

创建自定义统计脚本监控:

- Maker订单成交率
- 平均等待时间
- 平均费用节省
- 模型预测准确率
- 每日信号数量
- 持仓盈亏统计

---

## 常见问题

### 部署相关

**Q: 第一次执行会在什么时候?**

下一个整点。例如:
- 如果现在是10:25，下次执行在11:00
- 候选池筛选在明天早上09:00(UTC 01:00)首次执行

**Q: 如果没有候选池文件会怎样?**

小时级检测会输出:
```
候选池文件不存在: data/daily_scans/candidates_YYYYMMDD.csv
本小时无交易信号
```

需要手动运行一次:
```bash
python3 scripts/daily_candidate_scan.py --top-n 100
```

**Q: 如何临时禁用交易?**

方法1: 注释cron任务
```bash
crontab -e
# 在交易任务前加 # 号
```

方法2: 删除所有任务
```bash
bash deploy/clear_crontab.sh
```

方法3: 修改代码中的max_positions=0

**Q: 如何回滚到之前的配置?**

```bash
# 查看备份文件
ls -lt /tmp/crontab_backup_*.txt

# 恢复指定备份
crontab /tmp/crontab_backup_YYYYMMDD_HHMMSS.txt

# 验证
crontab -l
```

### 交易相关

**Q: Maker订单成交率低怎么办?**

调整参数:
```python
executor = MakerOrderExecutor(
    maker_offset_pct=0.05,  # 降低偏移量(当前0.10)
    max_wait_seconds=90     # 增加等待时间(当前60)
)
```

或直接修改 [scripts/live_maker_trader.py](scripts/live_maker_trader.py) 中的默认值。

**Q: 如何确认定时任务真的在运行?**

1. 查看系统日志:
   ```bash
   sudo grep CRON /var/log/syslog | grep Quant4Little | tail -10
   ```

2. 查看应用日志:
   ```bash
   ls -lth logs/
   ```

3. 等到整点后检查:
   ```bash
   tail -50 logs/cron_hourly.log
   ```

**Q: 持仓超过10个怎么办?**

系统会自动拒绝新订单:
```
当前持仓: 10
可用槽位: 0
跳过本次交易
```

需要等待已有持仓平仓后才能开新仓。

### 模型相关

**Q: 模型预测不准确怎么办?**

1. **收集更多数据**:
   ```bash
   python scripts/backtester.py --start 2024-01-01 --end 2025-01-31
   ```

2. **重新训练模型**:
   ```bash
   python scripts/modeling/train_ranker.py --epochs 50
   ```

3. **调整特征工程**: 修改 [scripts/modeling/features.py](scripts/modeling/features.py)

4. **禁用模型,使用传统排序**:
   - 编辑 [deploy/cron_hourly_detect.sh](deploy/cron_hourly_detect.sh)
   - 移除 `--use-model` 参数

**Q: 如何评估模型效果?**

```python
# 创建评估脚本
from scripts.lightweight_ranker import LightweightRanker
import pandas as pd

ranker = LightweightRanker()
signals = pd.read_csv("data/hourly_signals/signals_20250124_10.csv")

# 统计预测分布
print(signals["pred_class"].value_counts())
print(f"平均期望值: {signals['expected_value'].mean():.4f}")
print(f"Top-10平均期望值: {signals.head(10)['expected_value'].mean():.4f}")
```

**Q: 模型推理速度太慢怎么办?**

1. 转换为ONNX格式(3-5x加速)
2. 减少候选池大小(从300降到150)
3. 增加服务器CPU核心数
4. 使用更小的模型架构

### 数据相关

**Q: 数据下载失败怎么办?**

1. 检查网络连接和代理配置
2. 检查API密钥是否有效
3. 查看错误日志: `tail -50 logs/cron_candidates.log`
4. 手动重试: `python3 scripts/daily_candidate_scan.py --bottom-n 300`

**Q: 历史数据在哪里?**

```
data/
├── daily_klines/           # 日线数据
├── hourly_klines/          # 小时线数据
├── daily_scans/            # 每日候选池
├── hourly_signals/         # 每小时信号
└── backtest_trades.csv     # 历史交易(训练数据)
```

这些目录在 `.gitignore` 中被忽略，不会提交到Git。

**Q: 如何清理旧数据?**

```bash
# 清理60天前的K线数据
find data/daily_klines -mtime +60 -delete
find data/hourly_klines -mtime +60 -delete

# 清理7天前的候选池和信号
find data/daily_scans -mtime +7 -delete
find data/hourly_signals -mtime +7 -delete

# 保留交易记录(不要删除backtest_trades.csv!)
```

---

## 项目维护

### Git管理

```bash
# 初始化仓库
git init
git add .
git commit -m "初始化量化策略项目"

# 推送到GitHub
git remote add origin https://github.com/xxx/Quant4Little.git
git push -u origin main

# 拉取最新代码
git pull origin main
```

### 依赖更新

```bash
# 更新所有依赖
pip install --upgrade -r requirements.txt

# 更新单个依赖
pip install --upgrade ccxt
```

### 备份策略

定期备份重要文件:
- `models/` - 模型权重
- `data/backtest_trades.csv` - 交易记录
- `.env` - 环境变量(注意安全!)
- 日志文件(可选)

---

## 扩展功能

### 建议添加的功能

1. **持仓管理脚本**:
   - 自动监控持仓盈亏
   - 到期自动平仓
   - 止盈止损触发

2. **告警通知**:
   - Telegram/钉钉机器人
   - 异常订单告警
   - 盈亏提醒

3. **性能仪表板**:
   - Web界面展示统计
   - 实时持仓监控
   - 模型性能追踪

4. **自动重训**:
   - 定期用新数据重训模型
   - A/B测试新旧模型
   - 自动选择最优模型

5. **风控增强**:
   - 动态调整仓位大小
   - 市场情绪指标
   - 相关性分析

---

## 免责声明

本项目仅供学习研究使用，不构成任何投资建议。

**风险提示**:
- 加密货币交易具有极高风险，可能导致本金全部损失
- 历史回测不代表未来收益
- 模型预测不保证准确性
- 建议从小资金测试开始
- 务必在测试网充分验证后再使用实盘

使用本系统造成的任何损失，开发者不承担责任。

---

## 贡献与支持

欢迎提交Issue和Pull Request！

如有问题或建议，请在GitHub仓库提交Issue。

---

## 许可证

MIT License

---

**祝交易顺利！** 📈
